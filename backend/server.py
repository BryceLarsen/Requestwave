...
async def get_song_suggestions(
    musician_id: str = Depends(get_current_musician),
    show_id: Optional[str] = Query(None, description="Filter suggestions by show_id. If omitted, returns all suggestions.")
):
    """Get song suggestions for a musician, optionally filtered by show_id.
    
    - If show_id is provided: returns only suggestions for that specific show
    - If show_id is omitted: returns all suggestions (backward compatible)
    """
    try:
...
def _build_profile_public_response(musician, profile, songs_list):
    ...
    return {
        ...
        "cash_app_enabled": musician.get("cash_app_enabled", True),
        "zelle_enabled": musician.get("zelle_enabled", True),
        # Social media — profile values only. Blank profile fields render as blank
        # and never inherit from the master musician document.
        "instagram_username": profile.get("instagram_username") or None,
        "facebook_username": profile.get("facebook_url") or None,
        "tiktok_username": profile.get("tiktok_username") or None,
        "spotify_artist_url": profile.get("spotify_url") or None,
        "apple_music_artist_url": profile.get("apple_music_url") or None,
...
async def create_profile(profile_data: ProfileCreate, musician_id: str = Depends(get_current_musician)):
    ...
    profile_dict = {
        ...
        "instagram_username": profile_data.instagram_username or None,
        "tiktok_username": profile_data.tiktok_username or None,
        "facebook_url": profile_data.facebook_url or None,
        "spotify_url": profile_data.spotify_url or None,
        "apple_music_url": profile_data.apple_music_url or None,
        "website": profile_data.website or None,
