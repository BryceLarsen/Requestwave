...
- **Learn Later bookmark toggle (Feb 16, 2026)**: New per-song bookmark UX exposed in three views — Songs tab, On Stage view (Up Next + Live Requests), Requests tab (per-show grouped view + currentShow filtered view).
  - Backend: `POST /api/songs/{song_id}/learn-later` toggles song membership in the musician's "Learn Later" playlist (case-insensitive name match). Playlist is created lazily on first toggle, with `is_public=false, is_deleted=false`. Returns `{in_learn_later: bool, song_id, playlist_id}`. Auth + musician-scoping enforced (foreign/unknown song → 404). Other playlists are never touched (verified via regression test).
  - Backend: Authenticated `GET /api/songs` now attaches transient `in_learn_later: bool` to each song via a single set lookup of the Learn Later playlist's song_ids (no N+1). Public `GET /api/musicians/{slug}/songs` deliberately does NOT populate this field (no leak of musician's private bookmarks).
  - Frontend: Small bookmark SVG with "LL" text overlay; filled/yellow when bookmarked, outline/gray otherwise. Optimistic state update with axios-failure revert. Single source of truth is the `songs[]` state — every visible bookmark for the same song flips simultaneously. Helper `renderLearnLaterBookmark({songId|song, size})` no-ops when `song_id` isn't in the local catalog, which satisfies the spec's "only render when song matches catalog" rule on the Requests/OnStage views. `data-testid="learn-later-bookmark-<song_id>"`, `data-in-learn-later="true|false"`.
- **Misc small backend/UX (Feb–May 2026)**:
  - `register_musician` now auto-creates a default profile (slug from name, `is_default=true`) so newly registered musicians have a working profile/URL immediately.
  - Audience post-request success screen: "Skip / I'm all set" button now reads "Done" when the active profile has no tip links configured (paypal/venmo/cash_app/zelle).
  - All frontend audience-URL construction (profile card display, Copy URL, QR backend call) now derives the base URL from `window.location.origin` instead of a hardcoded `https://requestwave.app`, so URLs are correct regardless of environment.
  - Profile social link fields (instagram_username, tiktok_username, facebook_url, spotify_url, apple_music_url, website) no longer inherit/fallback to the musician document on the audience response. Blank profile field saves as `null` (create path coerces empty strings to None to match the existing update path).
  - Auto-show creation: added `_clear_stale_current_show()` helper invoked before each of the 3 auto-show entry points (POST /api/requests, POST /api/musicians/{slug}/requests, suggestion endpoint). If `current_show_id` on the profile/event points to a deleted show, the stale pointer is cleared at its source and the existing `if not current_show_id` guard re-fires — fixing the case where a profile would silently stamp requests with a dead show_id forever.
  - `get_unique_show_counts` $match now excludes `show_id: null` so requests with no show don't inflate the unique-show count via a phantom `null` entry in $addToSet.
- **Unassigned Requests panel (May 18, 2026)**: Requests tab gets a new collapsible "📥 Unassigned Requests (N)" panel at the bottom — only renders when ≥1 orphan exists; collapsed by default.
  - Backend: `GET /api/requests/unassigned` returns non-archived requests with null/missing/empty show_id, sorted by `created_at` desc. `POST /api/requests/{id}/assign-show` accepts `{show_id}`, sets both `show_id` and `show_name`; 400 on bad input, 404 on unknown request/show.
  - Frontend: per-orphan card shows song title, requester name, submitted timestamp + 3 actions: "Assign to Show" (dropdown sorted by created_at desc), "Archive" (existing endpoint), "Delete" (existing endpoint with `window.confirm`). Optimistic removal; panel disappears when list reaches zero.
- **Admin Panel (May 18, 2026)**: Hidden `/admin` route, not linked from anywhere, gated by `ADMIN_PASSWORD` env var.
  - Backend: `POST /api/admin/login` (returns JWT with role='requestwave-admin'); `require_admin` dependency. `GET /api/admin/users` lists all musicians with aggregated `profile_count`, `show_count`, `request_count`. `GET /api/admin/users/{musician_id}` returns musician + profiles (each with their shows + per-show request_count) + orphan_shows + unassigned_request_count. `GET /api/admin/users/{musician_id}/requests?show_id=<id|unassigned>` lists requests filtered by show. `POST /api/admin/requests/reassign` bulk-sets show_id+show_name. `POST /api/admin/requests/delete` bulk-deletes.
  - Frontend: `/app/frontend/src/AdminPanel.jsx` — separate component imported into App.js. Login form (sessionStorage token, persists until browser close), user table with name/email/registered/profile/show/request counts + filter input, expandable rows showing profiles + shows + Data Actions panel (filter by show or "Unassigned", multi-select + Reassign / Delete with `window.confirm`).
  - Env: `ADMIN_PASSWORD=changeme-admin-pwd` (set in `/app/backend/.env` — user should rotate before production).
